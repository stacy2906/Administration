...
{
	/// <summary>
	/// Класс 'admEssenceUsrRol'
	/// </summary>
	/// <remarks>Сущность - Роли пользователей</remarks>
	/// <design>Код класса сгенерирован программой 'CS Designer'</design>
	public class admEssenceUsrRol : datUnitEssence
	{
		#region = ДИЗАЙНЕРЫ
		...
		/// /// <remarks>Источник данных береться используемый по умолчанию</remarks>
		public admEssenceUsrRol(DELETETYPES pDeleteType) : base(pDeleteType)
		{
		}
		...
		/// <param name="pDeleteType">Порядок удаления записей из таблиц базы данных</param>
		public admEssenceUsrRol(string pDataSourceAlias, DELETETYPES pDeleteType) : base(pDataSourceAlias, pDeleteType)
		{
		}
		...
		protected override void _mObjectAssembly()
		{
			...
			__fTableDescription = admApplication.__oTunes.__mTranslate("Роли пользователей");
			__fTableName = "UsrRol";
			__fTableAlias = "UR";
		...
		public override bool __mCheckRecordFieldsFill(ref DataRow pDataRow, TRIGGERTYPEFORCHANGERECORD pTriggerType)
		{
			...
			int vcodUsrRol = Convert.ToInt32(pDataRow["codUsrRol"]); // Сортировка
			string vdsiUsrRol = Convert.ToString(pDataRow["dsiUsrRol"]); // Название

			#region 'codUsrRol' - Сортировка

			/// Если код не указан, выполняется его расчет
			if (vcodUsrRol <= 0)
			{
				pDataRow["codUsrRol" ] = datApplication.__oData.__mDataSourceGet(__fDataSourceAlias).__mCodeNew(__fTableName, vCLU, __fCodeNewCalculateType, 1);
			}
			...
			{
				if (datApplication.__oData.__mDataSourceGet(__fDataSourceAlias).__mTableRowsCountWhere(__fTableName, "codUsrRol = " + vcodUsrRol.ToString() + " and CLU != " + vCLU.ToString()) > 0)
				{
					_fTriggerErrorsDescriptions.Add(datApplication.__oTunes.__mTranslate("Учетный код уже используется"));
				}
			}

			#endregion 'codcodUsrRol' - Сортировка

			#region 'dsiUsrRol' - Название

			/// Если название не указано, формируется сообщение об ошибке: 'Название не указано'
			if (vdsiUsrRol.Length == 0)
			{
			...
			{
				if (datApplication.__oData.__mDataSourceGet(__fDataSourceAlias).__mNameExists(__fTableName, vdsiUsrRol.Trim(), vCLU) == true)
					_fTriggerErrorsDescriptions.Add(datApplication.__oTunes.__mTranslate("Название уже используется"));
			}

			#endregion 'dsiUsrRol' - Название
