...
namespace nlAdministration
{
	...
	/// Класс 'admEssenceSrv'
	/// </summary>
	/// <remarks>Сущность - Сервера</remarks>
	/// <design>Код класса сгенерирован программой 'CS Designer'</design>
	public class admEssenceSrv : datUnitEssence
	{
		#region = ДИЗАЙНЕРЫ
		...
		/// /// <remarks>Источник данных береться используемый по умолчанию</remarks>
		public admEssenceSrv(DELETETYPES pDeleteType) : base(pDeleteType)
		{
		}
		...
		/// <param name="pDeleteType">Порядок удаления записей из таблиц базы данных</param>
		public admEssenceSrv(string pDataSourceAlias, DELETETYPES pDeleteType) : base(pDataSourceAlias, pDeleteType)
		{
		}
		...
		protected override void _mObjectAssembly()
		{
			...
			__fTableDescription = admApplication.__oTunes.__mTranslate("Сервера");
			__fTableName = "Srv";
			__fTableAlias = "S";
		...
		public override bool __mCheckRecordFieldsFill(ref DataRow pDataRow, TRIGGERTYPEFORCHANGERECORD pTriggerType)
		{
			...
			int vcgzSrv = Convert.ToInt32(pDataRow["cgzSrv"]); // Сортировка
			string vdsiSrv = Convert.ToString(pDataRow["dsiSrv"]); // Название
			string vEml = Convert.ToString(pDataRow["Eml"]); // Электронная почта

			#region 'cgzSrv' - Сортировка

			/// Если сортировка не указана, выполняется её расчет
			if (vcgzSrv <= 0)
			{
				pDataRow["cgzSrv" ] = datApplication.__oData.__mDataSourceGet(__fDataSourceAlias).__mCodeNew(__fTableName, vCLU, __fCodeNewCalculateType, 1);
			}

			#endregion 'cgzSrv' - Сортировка

			#region 'dsiSrv' - Название
			...
			{
				if (datApplication.__oData.__mDataSourceGet(__fDataSourceAlias).__mNameExists(__fTableName, vdsiSrv.Trim(), vCLU) == true)
					_fTriggerErrorsDescriptions.Add(datApplication.__oTunes.__mTranslate("Название уже используется"));
			}

			#endregion 'dsiSrv' - Название
