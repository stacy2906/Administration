...
namespace CsProtocols.DATA.Importers
{
    /// <summary>
        ...
        public static List<string> __mProtocolsFoldersDiscover(string pStartDirectory)
        {
            ...
            try
            {
                ...
                foreach (DirectoryInfo vAppDirectory in vDirectory.GetDirectories())
                {
                    ...
                    string vPathDebug = Path.Combine(vAppDirectory.FullName, @"bin\Debug\PROTOCOLs");
                    string vPathRelease = Path.Combine(vAppDirectory.FullName, @"bin\Release\PROTOCOLs");

                    if (Directory.Exists(vPathDebug) == true)
                        vReturn.Add(vPathDebug);
                    if (Directory.Exists(vPathRelease) == true)
                        vReturn.Add(vPathRelease);
                }
            }
            catch
            {
